...
newTable <- arrange(newTable, Subject.ID, Activity)
...
newTable <- newTable %>% group_by(Activity,Subject.ID) %>% summarise_each(funs(mean))
write.table(newTable, "Output.txt", row.name = FALSE)
